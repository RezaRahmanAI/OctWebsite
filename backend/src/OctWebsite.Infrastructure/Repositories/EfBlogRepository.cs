...
internal sealed class EfBlogRepository(ApplicationDbContext dbContext) : IBlogRepository
{
    ...
    {
        var exists = await dbContext.BlogPosts.AnyAsync(item => item.Id == post.Id, cancellationToken);
        if (!exists)
        {
            return null;
        }
    ...
    {
        var entity = await dbContext.BlogPosts.FirstOrDefaultAsync(item => item.Id == id, cancellationToken);
        if (entity is null)
        {
            return false;
        }
